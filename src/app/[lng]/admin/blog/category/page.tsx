          ...
          {selected && nowEdit ? (
            <>
              <Input
                ...
                className="h-9 flex-1 bg-basic-0/90"
              />
              <div className="ml-auto flex items-center gap-2">
                ...
                <button
                  type="button"
                  className="inline-flex h-9 items-center justify-center rounded-lg border border-basic-3 bg-basic-0/85 px-3 text-xs font-semibold text-fg-3 transition-colors hover:bg-basic-2"
                  onClick={() => {
                    setValue('parent', category);
                    setValue('nowEdit', false);
                  }}
            ...
            <>
              ...
              {selected ? (
                <div className="ml-auto flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    className="inline-flex h-9 items-center justify-center gap-1 rounded-lg border border-danger-3 bg-danger-4 px-3 text-xs font-semibold text-danger-1 transition-colors hover:bg-danger-4 dark:border-danger-2/50 dark:bg-danger-2/20 dark:text-danger-4 dark:hover:bg-danger-2/30"
        ...
        {category.child?.length ? (
          <div
            className={cn(
              ...
              depth >= 1 ? 'border-basic-4' : 'border-basic-3',
            )}
          >
    ...
    <div className="mx-auto w-full max-w-5xl space-y-4 px-2 pb-24 pt-3 sm:px-3 md:px-4">
      ...
      <section className={cn(SERVICE_PANEL_SOFT, 'space-y-3 p-4')}>
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-fg-1">카테고리 트리</h2>
        ...
        <div className="space-y-2">
          {categories.length > 0 ? (
            ...
            <p className="rounded-lg border border-dashed border-basic-3 bg-basic-0/70 p-4 text-sm text-fg-5">
              등록된 카테고리가 없습니다.
            </p>
          )}
        </div>
      ...
      <section className={cn(SERVICE_PANEL_SOFT, SERVICE_CARD_INTERACTIVE, 'space-y-3 p-4')}>
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Input
            id="category"
            className="h-10 flex-1 bg-basic-0/90"
        ...
        {selectedParent ? (
          <button
            type="button"
            className="inline-flex h-9 w-fit items-center justify-center rounded-lg border border-basic-3 bg-basic-0/85 px-3 text-xs font-semibold text-fg-3 transition-colors hover:bg-basic-2"
            onClick={() => {
              setValue('parent', null);
              setValue('nowEdit', false);
            }}
