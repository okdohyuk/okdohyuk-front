...
import DdayCounterClient from '~/app/[lng]/(mobile)/dday-counter/components/DdayCounterClient';
...
export const generateMetadata: GenerateMetadata = async ({ params }) =>
  translationsMetadata({ params, ns: 'dday-counter' });

export default async function DdayCounterPage({ params }: LanguageParams) {
  const { lng } = await params;
  const language = lng as Language;

  const { t } = await getTranslations(language, 'dday-counter');
  ...
  return (
    <div className="space-y-4">
      <header className={cn(SERVICE_PANEL, 'relative overflow-hidden px-5 py-6 md:px-7 md:py-7')}>
        ...
        <Text variant="d2" color="basic-4" className="relative z-10 mt-2 max-w-3xl leading-relaxed">
          {t('description')}
        </Text>
      </header>
