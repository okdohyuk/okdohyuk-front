...
export default class UserTokenUtil {
  ...
  static setAccessToken(token: string) {
    ...
    try {
      ...
      logger.error('Error parsing access token payload for expiry:', e);
  ...
  static setRefreshToken(token: string) {
    ...
    try {
      ...
      logger.error('Error parsing refresh token payload for expiry:', e);
      // 페이로드 파싱 실패 시 기본 만료일 사용
    }
  ...
  static getUserInfo() {
    ...
    if (userInfoString) {
      try {
        ...
        logger.error('user_info 쿠키 파싱 오류:', e);
